...
2. Identify concrete complexity signals in the scoped code, such as duplicated logic, misleading names, unnecessary indirection, tangled control flow, or state that is harder to follow than the behavior requires. Separate a readability change from a semantic change.
