...
3. Choose the smallest safely recoverable change. Prefer reversibility when rollback preserves data and operational safety; when rollback would be destructive or unreliable, define a forward-fix or reconciliation path instead. Account for read and write cost, storage, transaction scope, connection pooling, concurrency, deadlocks, privilege boundaries, and row-level access policies. Stage backfills and cutovers when live consumers or large data sets are involved.
4. Validate with representative queries and data, plan or timing evidence, constraint and permission checks, migration rehearsal or recovery evidence, application tests, and reconciliation. Observe the running system after deployment when the change is live.
...
- Match transaction and migration strategy to lock duration, concurrency, failure recovery, and the ability to roll back, forward-fix, or reconcile safely.
...
- Do not run destructive migrations, bulk rewrites, privilege changes, role changes, or row-policy changes without explicit authorization, a defined scope, a recovery or forward-fix path, and a verification plan.
...
- Authorization for privileged or destructive actions, recovery or forward-fix strategy, reconciliation, monitoring, and unverified conditions are explicit when applicable.
