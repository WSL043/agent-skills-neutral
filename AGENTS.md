...
4. Keep exactly one workflow active for the current cognitive phase. Each workflow is self-contained for its outcome and completion evidence. When the blocking phase genuinely changes, stop using the prior workflow body and switch owners; do not compose workflow bodies or load another skill merely because it is adjacent.
...
- If a task spans phases, choose the skill that owns the current blocking phase. When a later phase becomes active, switch to its owner instead of keeping both workflow bodies in context.
