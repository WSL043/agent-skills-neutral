...
| Skill body delivered | A traced tool call names `skills/<name>/SKILL.md`, has either a recognized zero-exit output envelope or an exact supported host-native MCP completion with `isError=false`, and returns the complete normalized body in one model-visible text block whose manifest digest belongs to the unchanged served bundle | That the model adopted or followed the guidance, or which filesystem object produced equivalent bytes |
...
- Hosts that inject Skill bodies internally need their own adapter event. The Codex trace adapter only records activation when a recognized successful command result or exact supported host-native MCP completion returns the complete expected Skill body in model-visible tool output.
